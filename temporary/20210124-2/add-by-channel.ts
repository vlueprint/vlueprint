...
const main = async () => {
  const userLocalData = JSON.parse(fs.readFileSync("nokori.json", 'utf-8')) as {[key: string]: string }
  const parseret = parse('../../sparql-endpoint/toLoad/resource-VirtualBeing.ttl')
  let quads = parseret.quads
  const store = new N3.Store(quads)
  quads.forEach( q => {
    const replacedObj = q.object.value.replace("https://vlueprint.org/resource/", "")
    if( q.predicate.value == "https://vlueprint.org/schema/youtubeChannelName" && userLocalData[replacedObj] ) {
      const obj = literal(userLocalData[replacedObj])
      store.addQuad(q.subject, namedNode("https://vlueprint.org/schema/userlocalID"), obj)
      delete(userLocalData[replacedObj])
    }
  })
  parseret.quads = store.getQuads(null, null, null, null)
  await write(parseret)
  fs.writeFileSync("nokori.json", JSON.stringify(userLocalData, null , 2))
}
